	...
	public class GameManager : SingletonComponent<GameManager>
	{
		...
		private void BoardComplete()
		{
			...
			if (ActiveCategory != dailyPuzzleId)
			{
				...
				bool categoryCompleted = (GetCompletedLevelCount(categoryInfo) == categoryInfo.levelInfos.Count - 1);
		...
		// Checks if all the words can still be found on the board
		private bool CheckBoard()
		{
			...
			for (int i = 0; i < ActiveBoardState.words.Length; i++)
			{
				// If the word is found then continue to next word
				if (ActiveBoardState.foundWords[i])
				{
					continue;
				}

				bool[]	used	= new bool[ActiveBoardState.tileLetters.Length];
				bool	found	= false;

				for (int j = 0; j < ActiveBoardState.tileLetters.Length; j++)
				{
					if (ActiveBoardState.tileStates[j] == BoardState.TileState.UsedButNotFound &&
						ActiveBoardState.words[i][0] == ActiveBoardState.tileLetters[j])
					{
						if (CheckForWord(used, ActiveBoardState.words[i].Remove(0, 1), j))
						{
							found = true;
							break;
						}
		...
		private bool CheckForWord(bool[] used, string word, int fromIndex)
		{
			...
			int iStart	= Mathf.FloorToInt((float)fromIndex / (float)ActiveBoardState.wordBoardSize) - 1;
			int jStart	= (fromIndex % ActiveBoardState.wordBoardSize) - 1;
			int iEnd	= iStart + 3;
			int jEnd	= jStart + 3;
			...
			iEnd	= (iEnd > ActiveBoardState.wordBoardSize) ? ActiveBoardState.wordBoardSize : iEnd;
			jEnd	= (jEnd > ActiveBoardState.wordBoardSize) ? ActiveBoardState.wordBoardSize : jEnd;

			for (int i = iStart; i < iEnd; i++)
			{
				for (int j = jStart; j < jEnd; j++)
				{
					int tileIndex = i * ActiveBoardState.wordBoardSize + j;

					if (ActiveBoardState.tileStates[tileIndex] == BoardState.TileState.UsedButNotFound && 
						word[0] == ActiveBoardState.tileLetters[tileIndex] &&
						!used[tileIndex])
					{
						if (CheckForWord(used, word.Remove(0, 1), tileIndex))
						{
							return true;
						}
					}
