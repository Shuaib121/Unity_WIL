...
public class TextToSpeech : Singleton<TextToSpeech>
{
    ...
    private Action<bool> OnInitializationComplete;
    ...
    /// <param name="OnInitializationComplete">The method to call after completion or failure of the initialization process. A boolean is passed as an argument to the method which indicates whether initialization was successful or not.</param>
    public void Initialize(Locale language, float speed, float pitch, float volume, Action<bool> OnInitializationComplete)
    {
        Initialize();
        this.OnInitializationComplete = OnInitializationComplete;

        ttsEngine.Call("RegisterInitializationCompleteListener", this.gameObject.name, "InitializationComplete");

        ttsEngine.Call("InitializeWithSettings", "", this.gameObject.name, "OnError");
...
#region PRIVATE_METHODS
    ...
    private void OnError(string error)
    {
        ...
        else if (OnInitializationComplete != null)
        {
    ...
    public void InitializationComplete(string wasSuccessful)
    {
        if(OnInitializationComplete != null)
        {
            if (wasSuccessful.ToLower() == "true") { OnInitializationComplete.Invoke(true); }
            else { OnInitializationComplete.Invoke(false); }
